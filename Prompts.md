...
- Discussed the sprint's warning that a committed API key is an automatic
  fail, and confirmed that putting the key in a frontend `.env` (e.g.
  `VITE_API_KEY`) does not actually protect it — frontend env vars get
...
## PDF parsing
- Used pdf.js (via CDN) for browser-side PDF text extraction, so the
...
- Initial implementation used Google's Gemini API, as recommended by the
  ...
  Google Cloud project (not just one used-up quota).
...
- Decided against linking billing for a learning project and switched the
  ...
  individual free model gets retired or moved to paid (which appears to
  happen fairly often based on my research).
...
- Discussed why Netlify alone can't host the whole project: it serves
  ...
  for `client/`, Render (a Node-compatible host) for `server/`.
- Learned that Render assigns its own `PORT` and expects the app to read
  ...
  deploy attempt failed ("Not Found" at the platform level) because a
  manual `PORT=3000` environment variable was overriding Render's own
  assignment. Removed that variable and let the existing
  `process.env.PORT || 3000` fallback in `server.js` do its job correctly.
